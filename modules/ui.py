...
def create_ui():
    ...
    #   Imported here rather than at module scope: it imports modules.ui back.
    from modules import ui_img2img_panel
    ...
    with gr.Blocks(analytics_enabled=False, head=canvas_head) as img2img_interface:
        ...
        #   The whole generation panel lives in ui_img2img_panel so the LLM2img tab can
        #   build an identical one under its own tabname and ScriptRunner.
        img2img_panel = ui_img2img_panel.create_img2img_panel("img2img", scripts.scripts_img2img, toprow, dummy_component)
        img2img_generation_tab = img2img_panel.generation_tab
        output_panel = img2img_panel.output_panel

        img2img_args = dict(
            fn=wrap_gradio_gpu_call(modules.img2img.img2img, extra_outputs=[None, '', '']),
            _js="submit_img2img",
            inputs=img2img_panel.submit_inputs,
            outputs=[
                output_panel.gallery,
                output_panel.generation_info,
                output_panel.infotext,
    ...
    with gr.Blocks(theme=shared.gradio_theme, analytics_enabled=False, title="Stable Diffusion", head=canvas_head) as demo:
        ...
        settings.text_settings.change(fn=update_image_cfg_scale_visibility, inputs=[], outputs=[img2img_panel.components['image_cfg_scale']])
        demo.load(fn=update_image_cfg_scale_visibility, inputs=[], outputs=[img2img_panel.components['image_cfg_scale']])
