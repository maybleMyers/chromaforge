# taken from huggingface transformers and comfyui
import torch
...
class MLP(nn.Module):
    def __init__(self, dim, hidden_dim=None, dtype=None, device=None):
        super().__init__()
        ...
        self.c_fc1 = nn.Linear(dim, n_hidden, bias=False, dtype=dtype, device=device)
        self.c_fc2 = nn.Linear(dim, n_hidden, bias=False, dtype=dtype, device=device)
        self.c_proj = nn.Linear(n_hidden, dim, bias=False, dtype=dtype, device=device)
...
class SingleAttention(nn.Module):
    def __init__(self, dim, n_heads, mh_qknorm=False, dtype=None, device=None):
        super().__init__()
        ...
        # this is for cond
        self.w1q = nn.Linear(dim, dim, bias=False, dtype=dtype, device=device)
        self.w1k = nn.Linear(dim, dim, bias=False, dtype=dtype, device=device)
        self.w1v = nn.Linear(dim, dim, bias=False, dtype=dtype, device=device)
        self.w1o = nn.Linear(dim, dim, bias=False, dtype=dtype, device=device)

        self.q_norm1 = (
            MultiHeadLayerNorm((self.n_heads, self.head_dim), dtype=dtype, device=device)
            if mh_qknorm
            else nn.LayerNorm(self.head_dim, elementwise_affine=False, dtype=dtype, device=device)
        )
        self.k_norm1 = (
            MultiHeadLayerNorm((self.n_heads, self.head_dim), dtype=dtype, device=device)
            if mh_qknorm
    ...
    def forward(self, c):
        ...
        output = attention_function(q.permute(0, 2, 1, 3), k.permute(0, 2, 1, 3), v.permute(0, 2, 1, 3), self.n_heads, skip_reshape=True)
        c = self.w1o(output)
        return c
...
class DoubleAttention(nn.Module):
    def __init__(self, dim, n_heads, mh_qknorm=False, dtype=None, device=None):
        ...
        # this is for x
        self.w2q = nn.Linear(dim, dim, bias=False, dtype=dtype, device=device)
        self.w2k = nn.Linear(dim, dim, bias=False, dtype=dtype, device=device)
        self.w2v = nn.Linear(dim, dim, bias=False, dtype=dtype, device=device)
        self.w2o = nn.Linear(dim, dim, bias=False, dtype=dtype, device=device)
...
class MMDiTBlock(nn.Module):
    def __init__(self, dim, heads=8, global_conddim=1024, is_last=False, dtype=None, device=None):
        super().__init__()

        self.normC1 = nn.LayerNorm(dim, elementwise_affine=False, dtype=dtype, device=device)
        self.normC2 = nn.LayerNorm(dim, elementwise_affine=False, dtype=dtype, device=device)
        if not is_last:
            self.mlpC = MLP(dim, hidden_dim=dim * 4, dtype=dtype, device=device)
            self.modC = nn.Sequential(
                nn.SiLU(),
                nn.Linear(global_conddim, 6 * dim, bias=False, dtype=dtype, device=device),
            )
        else:
            self.modC = nn.Sequential(
                nn.SiLU(),
                nn.Linear(global_conddim, 2 * dim, bias=False, dtype=dtype, device=device),
            )

        self.normX1 = nn.LayerNorm(dim, elementwise_affine=False, dtype=dtype, device=device)
        self.normX2 = nn.LayerNorm(dim, elementwise_affine=False, dtype=dtype, device=device)
        self.mlpX = MLP(dim, hidden_dim=dim * 4, dtype=dtype, device=device)
        self.modX = nn.Sequential(
            nn.SiLU(),
            nn.Linear(global_conddim, 6 * dim, bias=False, dtype=dtype, device=device),
        )

        self.attn = DoubleAttention(dim, heads, dtype=dtype, device=device)
...
class DiTBlock(nn.Module):
    ...
    def __init__(self, dim, heads=8, global_conddim=1024, dtype=None, device=None):
        super().__init__()

        self.norm1 = nn.LayerNorm(dim, elementwise_affine=False, dtype=dtype, device=device)
        self.norm2 = nn.LayerNorm(dim, elementwise_affine=False, dtype=dtype, device=device)
        ...
        self.attn = SingleAttention(dim, heads, dtype=dtype, device=device)
        self.mlp = MLP(dim, hidden_dim=dim * 4, dtype=dtype, device=device)
...
class MMDiT(nn.Module):
    def __init__(
        ...
        self.t_embedder = TimestepEmbedder(global_conddim, dtype=dtype, device=device)
        self.cond_seq_linear = nn.Linear(cond_seq_dim, dim, bias=False, dtype=dtype, device=device)
        self.init_x_linear = nn.Linear(patch_size * patch_size * in_channels, dim, dtype=dtype, device=device)
        ...
        for idx in range(n_double_layers):
            self.double_layers.append(
                MMDiTBlock(dim, n_heads, global_conddim, is_last=(idx == n_layers - 1), dtype=dtype, device=device)
            )

        for idx in range(n_double_layers, n_layers):
            self.single_layers.append(
                DiTBlock(dim, n_heads, global_conddim, dtype=dtype, device=device)
            )
    ...
    def apply_pos_embeds(self, x, h, w):
        ...
        cur_dim = self.h_max
        pos_encoding = self.positional_encoding.reshape(1, cur_dim, cur_dim, -1).to(dtype=x.dtype, device=x.device)
    ...
    def forward(self, x, timestep, context, transformer_options={}, **kwargs):
        ...
        c = torch.cat([self.register_tokens.to(dtype=c.dtype, device=x.device).repeat(c.size(0), 1, 1), c], dim=1)
