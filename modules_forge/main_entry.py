...
# Per-tab model state - stores the model selection for each browser tab independently
# This allows different tabs to use different models without affecting each other
ui_txt2img_checkpoint_state: gr.State = None
ui_img2img_checkpoint_state: gr.State = None
...
ui_torch_compile_mode: gr.Dropdown = None
...
# Torch compile options
torch_compile_options = ['Disabled', 'reduce-overhead', 'max-autotune', 'default']
...
def make_checkpoint_manager_ui():
    global ui_checkpoint, ui_vae, ui_clip_skip, ui_forge_unet_storage_dtype_options, ui_forge_async_loading, ui_forge_pin_shared_memory, ui_forge_inference_memory, ui_forge_preset, ui_z_transformer_dtype, ui_z_vae_dtype, ui_z_text_encoder_dtype, ui_chromadct_fp8_mode, ui_torch_compile_mode
    ...
    # Torch compile options (for Chroma/Z-Image transformers)
    ui_torch_compile_mode = gr.Dropdown(label="Torch Compile", value=lambda: shared.opts.torch_compile_mode, choices=torch_compile_options)
    bind_to_opts(ui_torch_compile_mode, 'torch_compile_mode', save=True, callback=refresh_model_loading_parameters)
...
def on_preset_change(preset=None):
    ...
    if shared.opts.forge_preset == 'sd':
        ...
        return [
            ...
            gr.update(maximum=2048, value=getattr(shared.opts, "sd_i2i_height", 512)),                # ui_img2img_height
            ...
            gr.update(visible=False),                                                   # ui_torch_compile_mode
        ]
    ...
    if shared.opts.forge_preset == 'xl':
        ...
        return [
            ...
            gr.update(maximum=2048, value=getattr(shared.opts, "xl_i2i_height", 1024)),               # ui_img2img_height
    ...
    if shared.opts.forge_preset == 'flux':
        ...
        return [
            ...
            gr.update(maximum=2048, value=getattr(shared.opts, "flux_i2i_height", 1024)),             # ui_img2img_height
    ...
    if shared.opts.forge_preset == 'chroma':
        ...
        return [
            ...
            gr.update(visible=True, value=getattr(shared.opts, "torch_compile_mode", 'Disabled')),   # ui_torch_compile_mode
        ]
    ...
    return [
        ...
        gr.update(maximum=2048, value=ui_settings_from_file['img2img/Height/value']),  # ui_img2img_height
